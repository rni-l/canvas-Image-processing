...
const api = {
  ...
  // 获取忘记密码验证码
  async getforgettenPwdCode(params) {
    if (!params.email || !regexp.email.test(params.email)) {
    ...
    let code = ''
    for (let i = 0; i < 6; i++) {
      code += utils.getRandom(10, 0)
    }
    const emailData = await sendMsg(params, {
      subject: 'canvas-Image-processing, 修改密码的验证码',
      html: `验证码：${code}`
    })
